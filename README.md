...
Ft = observed market flow state at time t
...
flow imbalance = f(buy pressure, sell pressure)

## 5. Feature Conceptualization

The system operates on features such as:
...
* a probabilistic model (not deterministic)
* a microstructure-inspired system (not purely technical analysis)
* a trade-expectancy estimator (not a forecasting tool)

## 10. Required Data Hierarchy (Ideal vs Approximate)
...
* price prediction (Incorrect)
* indicator-based trading (Incorrect)

It is:

* a probabilistic evaluation of trade expectancy based on inferred or observed market flow imbalance (Correct)
